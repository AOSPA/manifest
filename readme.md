...
back for review. You should note that this can sometimes cause issues when building if a there's
a large change that spans across multiple projects and must be synced in all of them.

```bash
# Specify one or more projects by either name or path
# For example, either AOSPA/android_frameworks_base or just frameworks/base will do
$ repo sync PROJECT
```
